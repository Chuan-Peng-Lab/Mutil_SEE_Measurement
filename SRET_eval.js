...
var RJ_formal_words = addd(RJ_formal_index,RJ_formal_words,all_words);
...
var EW_instructions = {
  ...
  stimulus: `<div style="width:700px;">
    <p style='color:white;font-size:45px; line-height:30px;'>请您再次阅读实验要求</p>
    <p style='color:white;font-size:25px; line-height:30px;'><b>请回忆某一个亲密的朋友，你们至少2年前就认识了，并且最近2年经常见面。</b></p>
    <p style='color:white;font-size:25px; line-height:30px;'>在本实验中，您将会看到一些形容词。</p>
...
var RJ_formal1 = {
  ...
  on_finish: function(data){  
       ...
       if (data.response === '1') {
        ...
        data.key_press = "familiar";
      } else if (data.response === '3') {
        data.key_press = "old";
      }
      if (data.key_press === "new" && data.identity === "new") {
        data.correct = true;
      } else if ((data.key_press === "familiar" || data.key_press === "old") && data.identity === "old") {
        data.correct = true;
      } else {
        data.correct = false;
      }
...
var RJ_formal2 = {
  ...
  on_finish: function(data){  
    if(data.key_press === "f"){
      data.response = "self";
    }else if(data.key_press === "j"){
      data.response = "friend";
    }
    data.word = jsPsych.timelineVariable("Words", true);
...
var RJ_excercise={ timeline: [fixation,RJ_formal1,if_RJ],
